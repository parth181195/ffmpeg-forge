...
async function main() {
  ...
  try {
    ...
    console.log(
      `Current: ${suggestions.currentFormat} / ${suggestions.currentVideoCodec} / ${suggestions.currentAudioCodec}`
    );
    ...
    console.log('\n--- Compatibility Check ---');
    const compat = await ffmpeg.checkConversionCompatibility(testVideo, 'webm', 'vp9', 'opus');

    console.log(
      `${compat.sourceFormat}/${compat.sourceVideoCodec} → ${compat.targetFormat}/${compat.targetVideoCodec}`
    );
