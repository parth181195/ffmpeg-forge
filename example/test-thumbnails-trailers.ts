...
async function main() {
  ...
  trailer1.segments.forEach((seg, i) => {
    console.log(
      `    ${i + 1}. ${seg.startTime.toFixed(1)}s - ${seg.duration.toFixed(1)}s (${seg.reason})`
    );
  });
  console.log('');
  ...
  try {
    console.log('\nThumbnails:');
    execSync(`ls -lh ${outputDir}/*.{jpg,png} 2>/dev/null | awk '{print "  " $9 ": " $5}'`, {
      stdio: 'inherit',
    });
    console.log('\nTrailers:');
    execSync(`ls -lh ${trailersDir}/*.mp4 2>/dev/null | awk '{print "  " $9 ": " $5}'`, {
      stdio: 'inherit',
    });
  } catch {
