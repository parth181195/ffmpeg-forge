  ...
  if (hwInfo.available.length === 0) {
    ...
    hwInfo.available.forEach(hw => {
      ...
      if (codecs && codecs.length > 0) {
        console.log(
          `    Codecs: ${codecs.slice(0, 5).join(', ')}${codecs.length > 5 ? '...' : ''}`
        );
      }
    });
    console.log('');
        ...
        onProgress: progress => {
          if (progress?.percent !== undefined) {
            process.stdout.write(
              `\r  ⏳ CPU: ${progress.percent.toFixed(1)}% | ${progress.currentFps || 0} fps`
            );
          }
        },
      }
    ...
    await ffmpeg.convert(
      ...
      },
      {
        ...
        onProgress: progress => {
          if (progress?.percent !== undefined) {
            process.stdout.write(
              `\r  ⏳ GPU: ${progress.percent.toFixed(1)}% | ${progress.currentFps || 0} fps`
            );
          }
        },
      }
