...
export class Concatenation {
   ...
   */
  private async simpleConcatenate(inputInfos: any[], config: ConcatenationConfig): Promise<ConcatenationResult> {
   ...
   */
  private async complexConcatenate(inputInfos: any[], config: ConcatenationConfig): Promise<ConcatenationResult> {
    const inputPaths = inputInfos.map(info => getInputPath(info));
    ...
    if (config.normalize?.enabled || config.transitions?.enabled) {
      ...
      const filter = `${videoInputs}concat=n=${inputPaths.length}:v=1:a=0[outv];` +
